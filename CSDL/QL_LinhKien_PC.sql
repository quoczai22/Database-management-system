...
-- 1. View của Linh Kiện
CREATE OR ALTER VIEW v_LinhKien AS
SELECT 
    LK.MaLK, LK.TenLK, LK.NgayNhap, LK.TGBH, 
    L.TenLoai, N.TenNSX, LK.DVT, LK.SoLuongTon, LK.DonGiaBan, LK.NgungKinhDoanh
FROM LinhKien LK
JOIN LoaiLK L ON LK.MaLoai = L.MaLoai
JOIN NhaSanXuat N ON LK.MaNSX = N.MaNSX;
GO

-- 2. View của Loại Linh Kiện
CREATE OR ALTER VIEW v_LoaiLK AS
SELECT MaLoai, TenLoai, MoTa
FROM LoaiLK;
GO

-- 3. View của Nhân Viên
CREATE OR ALTER VIEW v_NhanVien AS
SELECT 
    MaNV, TenNV, GioiTinh, NgaySinh, SDT, ChucVu, Quyen, Email, NgayVaoLam, DaNghiViec
FROM NhanVien;
GO

-- 4. View của Khách Hàng
CREATE OR ALTER VIEW v_KhachHang AS
SELECT 
    MaKH, TenKH, DChi, SDT, Email
FROM KhachHang;
GO
...
SELECT 
    HD.MaHD, HD.NgayHD, 
    KH.TenKH, NV.TenNV, 
    HD.TongTien, HD.TrangThai, HD.PhuongThucThanhToan, HD.NgayThanhToan
FROM HoaDon HD
...
-- THÊM Linh Kiện
CREATE OR ALTER PROC sp_ThemLinhKien
    @MaLK CHAR(6), @TenLK NVARCHAR(50), @NgayNhap DATE, @TGBH TINYINT,
    @MaLoai CHAR(3), @MaNSX CHAR(5), @DVT NVARCHAR(10), @SoLuongTon INT, @DonGiaBan INT
AS
BEGIN
    INSERT INTO LinhKien (MaLK, TenLK, NgayNhap, TGBH, MaLoai, MaNSX, DVT, SoLuongTon, DonGiaBan)
    VALUES (@MaLK, @TenLK, @NgayNhap, @TGBH, @MaLoai, @MaNSX, @DVT, @SoLuongTon, @DonGiaBan);
END;
GO

-- SỬA Linh Kiện
CREATE OR ALTER PROC sp_SuaLinhKien
    @MaLK CHAR(6), @TenLK NVARCHAR(50), @NgayNhap DATE, @TGBH TINYINT,
    @MaLoai CHAR(3), @MaNSX CHAR(5), @DVT NVARCHAR(10), @SoLuongTon INT, @DonGiaBan INT
AS
BEGIN
    UPDATE LinhKien
    SET TenLK = @TenLK, NgayNhap = @NgayNhap, TGBH = @TGBH, MaLoai = @MaLoai, 
        MaNSX = @MaNSX, DVT = @DVT, SoLuongTon = @SoLuongTon, DonGiaBan = @DonGiaBan
    WHERE MaLK = @MaLK;
END;
GO

-- XÓA Linh Kiện (Soft Delete: Đánh dấu ngừng kinh doanh)
CREATE OR ALTER PROC sp_XoaLinhKien
    @MaLK CHAR(6)
AS
BEGIN
    UPDATE LinhKien SET NgungKinhDoanh = 1 WHERE MaLK = @MaLK;
END;
GO

-- THÊM Loại LK
CREATE OR ALTER PROC sp_ThemLoaiLK
    @MaLoai CHAR(3), @TenLoai NVARCHAR(40), @MoTa NVARCHAR(100)
AS
BEGIN
    INSERT INTO LoaiLK (MaLoai, TenLoai, MoTa) VALUES (@MaLoai, @TenLoai, @MoTa);
END;
GO

-- SỬA Loại LK
CREATE OR ALTER PROC sp_SuaLoaiLK
    @MaLoai CHAR(3), @TenLoai NVARCHAR(40), @MoTa NVARCHAR(100)
AS
BEGIN
    UPDATE LoaiLK SET TenLoai = @TenLoai, MoTa = @MoTa WHERE MaLoai = @MaLoai;
END;
GO

-- XÓA Loại LK
CREATE OR ALTER PROC sp_XoaLoaiLK
    @MaLoai CHAR(3)
AS
BEGIN
    DELETE FROM LoaiLK WHERE MaLoai = @MaLoai;
END;
GO

-- THÊM Nhân Viên
CREATE OR ALTER PROC sp_ThemNhanVien
    @MaNV CHAR(6), @TenNV NVARCHAR(40), @GioiTinh NVARCHAR(5), @NgaySinh DATE,
    @SDT CHAR(10), @ChucVu NVARCHAR(30), @Quyen NVARCHAR(20), @Email VARCHAR(50), @NgayVaoLam DATE
AS
BEGIN
    INSERT INTO NhanVien (MaNV, TenNV, GioiTinh, NgaySinh, SDT, ChucVu, Quyen, Email, NgayVaoLam)
    VALUES (@MaNV, @TenNV, @GioiTinh, @NgaySinh, @SDT, @ChucVu, @Quyen, @Email, @NgayVaoLam);
END;
GO

-- SỬA Nhân Viên
CREATE OR ALTER PROC sp_SuaNhanVien
    @MaNV CHAR(6), @TenNV NVARCHAR(40), @GioiTinh NVARCHAR(5), @NgaySinh DATE,
    @SDT CHAR(10), @ChucVu NVARCHAR(30), @Quyen NVARCHAR(20), @Email VARCHAR(50), @NgayVaoLam DATE
AS
BEGIN
    UPDATE NhanVien
    SET TenNV = @TenNV, GioiTinh = @GioiTinh, NgaySinh = @NgaySinh, SDT = @SDT,
        ChucVu = @ChucVu, Quyen = @Quyen, Email = @Email, NgayVaoLam = @NgayVaoLam
    WHERE MaNV = @MaNV;
END;
GO

-- XÓA Nhân Viên (Soft Delete: Đánh dấu đã nghỉ việc)
CREATE OR ALTER PROC sp_XoaNhanVien
    @MaNV CHAR(6)
AS
BEGIN
    UPDATE NhanVien SET DaNghiViec = 1 WHERE MaNV = @MaNV;
END;
GO

-- THÊM Khách Hàng
CREATE OR ALTER PROC sp_ThemKhachHang
    @MaKH CHAR(6), @TenKH NVARCHAR(30), @DChi NVARCHAR(50), @SDT CHAR(10), @Email VARCHAR(50)
AS
BEGIN
...
-- SỬA Khách Hàng
CREATE OR ALTER PROC sp_SuaKhachHang
    @MaKH CHAR(6), @TenKH NVARCHAR(30), @DChi NVARCHAR(50), @SDT CHAR(10), @Email VARCHAR(50)
AS
BEGIN
    UPDATE KhachHang
    SET TenKH = @TenKH, DChi = @DChi, SDT = @SDT, Email = @Email
    WHERE MaKH = @MaKH;
END;
GO
...
CREATE OR ALTER PROC sp_ThemHoaDon
    @MaHD CHAR(5), @NgayHD DATE, @MaKH CHAR(6), @MaNV CHAR(6), 
    @TrangThai NVARCHAR(30), @PhuongThucThanhToan NVARCHAR(50), @NgayThanhToan DATE
AS
BEGIN
    INSERT INTO HoaDon (MaHD, NgayHD, MaKH, MaNV, TrangThai, PhuongThucThanhToan, NgayThanhToan)
    VALUES (@MaHD, @NgayHD, @MaKH, @MaNV, @TrangThai, @PhuongThucThanhToan, @NgayThanhToan);
END;
GO

-- SỬA Hóa Đơn
CREATE OR ALTER PROC sp_SuaHoaDon
    @MaHD CHAR(5), @NgayHD DATE, @MaKH CHAR(6), @MaNV CHAR(6), 
    @TrangThai NVARCHAR(30), @PhuongThucThanhToan NVARCHAR(50), @NgayThanhToan DATE
AS
BEGIN
    UPDATE HoaDon
    SET NgayHD = @NgayHD, MaKH = @MaKH, MaNV = @MaNV, TrangThai = @TrangThai, 
        PhuongThucThanhToan = @PhuongThucThanhToan, NgayThanhToan = @NgayThanhToan
    WHERE MaHD = @MaHD;
END;
GO
...
AS
BEGIN
    -- Cần xóa Chi Tiết Hóa Đơn trước khi xóa Hóa Đơn để tránh lỗi FK
